...
void rotate( mat& A, mat& R, int k, int l, int n )
{
    double sind, cosd;
    double tand, tau;
    if ( A(k,l) != 0.0 ) { //Making sure not devide by zero
        tau = (A(l,l) - A(k,k))/(2*A(k,l)); //The rotation angle
        if ( tau > 0 ) {
