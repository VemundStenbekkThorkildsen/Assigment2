...
=======
>>>>>>> 5846839f569697a851d7dbb2d16611b378390db5
...
<<<<<<< HEAD
Two tests were implemented using user input. The first one is testing if the rotation matrix R is orthogonal so that $R^{T}R = I$. This was calculated by approximately comparing $R^{T}R$ to I. The second test was comparing the numerically calculated eigenvalues of a matrix S 
...
\begin{center}
\includegraphics[scale=0.5]{eigomg001.jpg}
\includegraphics[scale=0.5]{eigomg05.jpg}
\includegraphics[scale=0.5]{eigomg1.jpg}
\includegraphics[scale=0.5]{eigomg5.jpg}
\end{center}
...
\end{center}
\newpage
In the first case we established a sensible value of ${\rho}_{max}$. We were given the analytic solution for the three lowest eigenvalues. By looking at Figure 1,2,3 and 4, it soon becomes clear that using ${\rho}_{max}<5$ does not give the correct eigenvalues. Figure 5 shows this problem in another context. We are effectively trying to push our probability distribution into  the boundaries of $\rho$  
